...
# IPv6 — attempt separately; silently skip if IPv6 is not active
if ($ipv6IPs.Count -gt 0) {
    try {
        ...
        Write-Warn "IPv6 firewall rules skipped (interface may not support IPv6): $_"
    }
}
...
try {
    Set-RegValue $chromePolicyPath 'DnsOverHttpsMode' 'off'
    Set-RegValue $chromePolicyPath 'DnsOverHttpsTemplates' ''
...
try {
    Set-RegValue $edgePolicyPath 'DnsOverHttpsMode' 'off'
    Set-RegValue $edgePolicyPath 'DnsOverHttpsTemplates' ''
...
try {
    Set-RegValue $firefoxPolicyPath 'Enabled' 0 'DWord'
    Set-RegValue $firefoxPolicyPath 'Locked'  1 'DWord'
...
try {
    Set-RegValue $bravePolicyPath 'DnsOverHttpsMode' 'off'
