...
function startMinistrySync(getSettings, setSettings, logAudit, logger) {
    return setInterval(async () => {
        ...
        if (!settings.ministry_mode || !settings.remote_policy_url) return;
        ...
        try {
            ...
            // Push local audit logs to the ministry server
            await logger.syncToMinistryServer(settings.remote_policy_url);
        } catch (e) {
            logAudit(
